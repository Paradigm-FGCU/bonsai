...
public class ResultActivity extends CActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (RunningInfo.getTimedQuiz()) {
			// Get Card Times
			cardTime = b.getIntArray("cardTime");

			scoreT = String
					.format("Score: %d out of %d  %3.0f%%\n\nQuiz Results: \nAverage Quiz Score: %3.0f%%\nTotal Quiz Time: %ds",
							score, deckSize, quizPercent, quizAve,cardTime[deckSize]);
		} else
			scoreT = String
					.format("Score: %d out of %d  %3.0f%%\n\nQuiz Results: \nAverage Quiz Score: %3.0f%%",
		...
		for (int i = 0; i < quizResults.length; i++) {
			...
			String parText = String.format("Q%3d:%-14sAvg: %3.0f%%", qNum,
					cardCorrectness, cardCorrectPercent);
			String childText;
			if (RunningInfo.getTimedQuiz())
				childText = String.format("Time Used:%d s\n\nTerm:\n%s\n\n",
						cardTime[i], cards.get(i).getAnswer());
			else
				childText = String.format("Term:\n%s\n\n", cards.get(i)
						.getAnswer());

			if (!cards.get(i).getQuestion().equals(selectedAns[i])) {
				childText = childText
						+ String.format("Your Answer:\n%s\n\n", selectedAns[i]);
			}
			childText = childText
					+ String.format("Correct Answer:\n%s\n\n", cards.get(i)
							.getQuestion());
		...
		dbHelper.updateDeckStats(deck.getDeckId(), deck.getQuizAverage(),
				deck.getQuizCount());
		dbHelper.close();
	...
	public boolean onKeyDown(int keycode, KeyEvent event) {
		if (keycode == KeyEvent.KEYCODE_BACK) {
			...
			Intent i = new Intent(ResultActivity.this, MainMenuActivity.class);
			startActivity(i);
